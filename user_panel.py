...
def demo():
    ...
    head = Label(window, text="Database Entry", font=("arial", 20, "bold")).pack()
    ...
    label3 = Label(window, text="Start_date_date", font=("arial", 10, "bold")).pack()
...
def SSL():
    ...
    cursor = connect.cursor()
    cursor.execute("DROP TABLE IF EXISTS sl")
    cursor.execute('CREATE TABLE s_ssl (s_id VARCHAR(25),s_name VARCHAR(50),Address VARCHAR(100),'
                   'Start_date DATE,Exp_date DATE)')
    cursor.execute("INSERT INTO s_ssl (s_id,s_name,Address,Start_date,Exp_date) VALUES (%s,%s,%s,%s,%s)",
                    (uid, name, Add, s_date, e_date))
...
def checkone():
    ...
    for i in row:
        ...
        if compare > timedelta():
            ...
            notification_manager.warning("warning! your account may discontinued from tomarrow")
...
def Ayush():
    ...
    def button():
        database = Label(window, text="DATABASE MANAGEMENT", font=("arial", 10, "bold")).pack()

        domain = Button(window, text="Domain", font=("arial", 10, "bold"), command=demo, height=5, width=10,
                        fg="green").pack(side=RIGHT)
        host = Button(window, text="Host", font=("arial", 10, "bold"), command=demo1, height=5, width=10,
                      fg="green").pack()
        sl = Button(window, text="SSL", font=("arial", 10, "bold"), command=demo2, height=5, width=10,
                    fg="green").pack(side=LEFT)
        quit = Button(window, text="QUIT", font=("arial", 10, "bold"), width=10, height=5, fg="red",
                      command=exit).pack(side=BOTTOM)
