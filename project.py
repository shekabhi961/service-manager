...
def demo():
    ...
    def dem():
        head = Label(window, text="Domain Database", font=("arial", 20, "bold")).pack()

        label_3 = Label(window, text="Id", font=("arial", 10, "bold")).pack()
        entry_3 = Entry(window, textvariable=id).pack()

        label = Label(window, text="Name", font=("arial", 10, "bold")).pack()
        entry = Entry(window, textvariable=nm).pack()

        label_1 = Label(window, text="Address", font=("arial", 10, "bold")).pack()
        entry_1 = Entry(window, textvariable=add).pack()

        label_2 = Label(window, text="Exp_date", font=("arial", 10, "bold")).pack()
        entry_2 = Entry(window, textvariable=exp_date).pack()

        label3 = Label(window, text="Start_date", font=("arial", 10, "bold")).pack()
        entry3 = Entry(window, textvariable=start_date).pack()

        var = IntVar()
        Gender = Label(window, text='Status', font=("arial", 10, "bold")).pack()
        radio = Radiobutton(window, text="Purchased", variable=var, value=0).pack()
        radio_1 = Radiobutton(window, text="Rented", variable=var, value=1).pack()

        submit = Button(window, text="submit", font=("arial", 10, "bold"), command=domain).pack()
        btn1 = Button(window, text="quit", font=("arial", 10, "bold"), command=quit).pack()
...
def demo1():
    ...
    def dem2():
        head = Label(window, text="Host Database", font=("arial", 20, "bold")).pack()

        label_3 = Label(window, text="Id", font=("arial", 10, "bold")).pack()
        entry_3 = Entry(window, textvariable=id1).pack()

        label = Label(window, text="Name", font=("arial", 10, "bold")).pack()
        entry = Entry(window, textvariable=nm1).pack()

        label_1 = Label(window, text="Address", font=("arial", 10, "bold")).pack()
        entry_1 = Entry(window, textvariable=add1).pack()

        label_2 = Label(window, text="Exp_date", font=("arial", 10, "bold")).pack()
        entry_2 = Entry(window, textvariable=exp_date1).pack()

        label3 = Label(window, text="Start_date", font=("arial", 10, "bold")).pack()
        entry3 = Entry(window, textvariable=start_date1).pack()
        ...
        submit = Button(window, text="submit", font=("arial", 10, "bold"), command=Host).pack()
        btn1 = Button(window, text="quit", font=("arial", 10, "bold"), command=quit).pack()
...
def demo2():
    ...
    def dem3():
        head = Label(window, text="SSL Database", font=("arial", 20, "bold")).pack()

        label_3 = Label(window, text="Id", font=("arial", 10, "bold")).pack()
        entry_3 = Entry(window, textvariable=id2).pack()

        label = Label(window, text="Name", font=("arial", 10, "bold")).pack()
        entry = Entry(window, textvariable=nm2).pack()

        label_1 = Label(window, text="Address", font=("arial", 10, "bold")).pack()
        entry_1 = Entry(window, textvariable=add2).pack()

        label_2 = Label(window, text="Exp_date", font=("arial", 10, "bold")).pack()
        entry_2 = Entry(window, textvariable=exp_date2).pack()

        label3 = Label(window, text="Start_date", font=("arial", 10, "bold")).pack()
        entry3 = Entry(window, textvariable=start_date2).pack()
        ...
        submit = Button(window, text="submit", font=("arial", 10, "bold"), command=SSL).pack()
        btn1 = Button(window, text="quit", font=("arial", 10, "bold"), command=quit).pack()
...
def checkone():
    ...
    for i in row:
        ...
        if compare > timedelta():
            ...
            var = cursor.execute("SELECT domain_name FROM domain ")
            ...
            for i in varm:
                ...
                if resp:
                    send_email("shekabhi961@gmail.com", "9602311961", "aayush@indiadox.com", "AntMan",
                           "Hey!!" + res + str(compare.days) + " is expiring today, Update Immediately " )

                    notification_manager.warning("warning! your account may discontinued from tomorrow")

        elif compare < timedelta():
            vara = cursor.execute("SELECT domain_name FROM domain ")
            ...
            for i in vare:
                ...
                if resps:
                    send_email("shekabhi961@gmail.com", "9602311961", "aayush@indiadox.com", "AntMan",
                           "Hey!!" + res + str(compare.days) + " is expired, Please Contact Service Provider " )
...
def checktwo():
    ...
    for i in row:
        ...
        if compare > timedelta():
            var = cursor.execute("SELECT host_name FROM host ")
            varp = cursor.fetchall()
            for i in varp:
                res = i[0]
                print(res)
            ...
            vars = cursor.execute("SELECT host_name FROM host ")
            varm = cursor.fetchall()
            for i in varm:
                resp = i[0]
                print(resp)
            ...
            vara = cursor.execute("SELECT host_name FROM host ")
            vare = cursor.fetchall()
            for i in vare:
                resps = i[0]
                print(resps)
...
def checkthree():
    ...
    for i in row:
        ...
        if compare > timedelta():
            var = cursor.execute("SELECT s_name FROM sl ")
            varp = cursor.fetchall()
            for i in varp:
                res = i[0]
                print(res)
                ...
                if res:
                    notification_manager.info(str(compare.days) + " days remaining, " + " your account is is expiring soon")
                    send_email("shekabhi961@gmail.com", "9602311961", "aayush@indiadox.com", "AntMan",
                               "Hey!!" + res + str(compare.days) + " remaining only, license is expiring soon ")
            ...
            vars = cursor.execute("SELECT s_name FROM sl ")
            varm = cursor.fetchall()
            for i in varm:
                resp = i[0]
                print(resp)
            ...
            vara = cursor.execute("SELECT s_name FROM sl ")
            vare = cursor.fetchall()
            for i in vare:
                resps = i[0]
                print(resps)
